...
from ..old import gamedefine, resourceGain
    ...
    for i in costs:
        if i["what"] in list(resourceGain.data.gainPerSecond.keys()):
            if i["amount"] == "atMarketPrice":
                itemToCheckFor = getCurrentInternalMultiLevelUpgrade(automation)["idleGenerator"]["whatItGives"][0][
                    "what"
                ]
            ...
            print(
                f"{i} is not in resourceGain.data.gainPerSecond, so cannot be calculated for automation {automation}."
            )
...
def canAffordAutomationTask(automation: str) -> bool:
    ...
    for i in costs:
        if i["amount"] == "atMarketPrice":
            if (
                gamedefine.gamedefine.amounts[i["what"]]
                < gamedefine.gamedefine.itemInternalDefine[i["what"]]["whatItCosts"][0]["amount"]
            ):
                ongoing = False
...
def updateAutomationStatus(automation: str) -> None:
    ...
    if currentUpgradeDict["type"] == "idleGenerator":
        ...
        if idleGenDict["equationType"] == "timeEquation":
            gamedefine.gamedefine.automationDetails[automation]["timeToWait"] = numberLogic.evaluateCostEquation(
                idleGenDict["timeEquation"], gamedefine.gamedefine.automationLevels[automation]
            )
            for i in range(len(idleGenDict["whatItGives"])):
                gamedefine.gamedefine.automationDetails[automation]["whatItGives"][i]["amount"] = idleGenDict[
                    "whatItGives"
                ][i]["amount"]

            if idleGenDict["withRequirement"]:
                for i in range(len(idleGenDict["whatItCosts"])):
                    gamedefine.gamedefine.automationDetails[automation]["whatItCosts"][i]["amount"] = idleGenDict[
                        "whatItCosts"
                    ][i]["amount"]
            ...
            for i in range(len(idleGenDict["whatItGives"])):
                amount = numberLogic.evaluateCostEquation(
                    idleGenDict["amountEquation"][i]["equation"], gamedefine.gamedefine.automationLevels[automation]
                )
            ...
            if idleGenDict["withRequirement"]:
                for i in idleGenDict["whatItCosts"]:
                    try:
                        ...
                        continue
                    amount = numberLogic.evaluateCostEquation(
                        i["equation"], gamedefine.gamedefine.automationLevels[automation]
                    )
                    i["amount"] = amount
            ...
            if internalDefine["withRequirement"]:
                ...
                if canAffordAutomationTask(automation):
                    ...
                    for i in gamedefine.gamedefine.automationDetails[automation]["whatItCosts"]:
                        if i["amount"] == "atMarketPrice":
                            itemToCheckFor = getCurrentInternalMultiLevelUpgrade(automation)["idleGenerator"][
                                "whatItGives"
                            ][0]["what"]
                            price = int(
                                gamedefine.gamedefine.itemInternalDefine[itemToCheckFor]["whatItCosts"][0]["amount"]
                            )
...
def getCostWithIndex(index: int, automation: str) -> int:
    ...
    if type(item["amount"]) == str:
        ...
        raise TypeError(
            f"Type of amount in upgrade {automation} with level {gamedefine.gamedefine.automationLevels[automation]} has the wrong type."
        )
...
def parseUsefulDescription(automation) -> str:
    ...
    if currentInternalDict["type"] == "idleGenerator":
        ...
        if currentVisualDict["usefulDescriptionBlank"] == "tickTime":
            currentDec = currentDec.replace(
                "%%%", str(round(gamedefine.gamedefine.automationDetails[automation]["timeToWait"] / 1000, 3))
            )
            ...
            currentDec = currentDec.replace(
                "%%%", str(round(gamedefine.gamedefine.automationDetails[automation]["whatItGives"][0]["amount"]))
            )
...
def getFutureDescription(automation):
    futureInternalDict = getCurrentInternalMultiLevelUpgrade(
        automation, gamedefine.gamedefine.automationLevels[automation] + 1
    )
    futureVisualDict = getCurrentVisualMultiLevelUpgrade(
        automation, gamedefine.gamedefine.automationLevels[automation] + 1
    )
    ...
    if futureInternalDict["type"] == "idleGenerator":
        ...
        if futureVisualDict["usefulDescriptionBlank"] == "tickTime":
            futureNum = numberLogic.evaluateCostEquation(
                futureInternalDict["idleGenerator"]["timeEquation"],
                gamedefine.gamedefine.automationLevels[automation] + 1,
            )
            ...
            futureNum = numberLogic.evaluateCostEquation(
                futureInternalDict["idleGenerator"]["amountEquation"][0]["equation"],
                gamedefine.gamedefine.automationLevels[automation] + 1,
            )
...
def getAutomationName(automation):
    return gamedefine.gamedefine.automationVisualDefine[automation][getCurrentMultiLevelUpgradeIndex(automation)][
        "visualName"
    ]
