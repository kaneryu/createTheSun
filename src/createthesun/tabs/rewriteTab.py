...
class automationBlock(QFrame):
    ...
    def updateDisplay(self):
        ...
        if gamedefine.gamedefine.automationDisabledState[self.name][0]:
            self.upgradeLabel.setText(
                f"Disabled with {gamedefine.gamedefine.automationDisabledState[self.name][1]} seconds left | {automationGameLogic.parseAutomationName(self.name)}"
            )
        else:
            self.upgradeLabel.setText(automationGameLogic.parseAutomationName(self.name))
