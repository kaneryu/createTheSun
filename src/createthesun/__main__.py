        ...
        def inner():
            ...
            if time.time() * 1000 - self.lastUpdateTime > 500:  # every 1/2 second
                ...
                observerModel.callEvent(
                    observerModel.Observable.ITEM_OBSERVABLE, observerModel.ObservableCallType.TIME, ""
                )
                observerModel.callEvent(
                    observerModel.Observable.AUTOMATION_OBSERVABLE, observerModel.ObservableCallType.TIME, ""
                )
                observerModel.callEvent(
                    observerModel.Observable.TIME_OBSERVABLE, observerModel.ObservableCallType.TIME, ""
                )
    ...
    if not updateCheck():  # if not on the latest version
        ...
        if not os.path.exists(command):
            error_dialog = dialogs.errorDialog(
                "Error",
                r"You are not on the latest version and the launcher is missing. Please download the latest version from [https:\\github.com\KaneryU\createTheSun](https:\\github.com\KaneryU\createTheSun)",
            )
...
def main():
    ...
    splashScreen.showMessage(
        "Loading...", Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignCenter, Qt.GlobalColor.white
    )
    ...
    if window.threadpool.maxThreadCount() < 2:
        error_dialog = dialogs.errorDialog(
            "Error", "You have less than 2 available threads. At least 2 threads are needed to run the game"
        )
        error_dialog.exec()
        sys.exit()
