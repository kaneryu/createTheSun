...
def save(
    export: bool = False,
    exportEncoded: bool = False,
    notify: bool = True,
    slot: int = 0,
    noProgSaveCreation: bool = False,
    blank: bool = False,
):  # type: ignore
    ...
    areYouSureDialog = dialogs.yesNoDialog(
        "Save", "Are you sure you want to save? This will overwrite the current save in that slot.", preventClose_=True
    )
    global selectedSlot
    ...
    if not blank:
        ...
        try:
            ...
            dialogs.errorDialog(
                "Error", f"There was an error saving your game. Please report this to the developer:\n{e}"
            ).exec()
        ...
        blankSave = b64Encode(
            json.dumps(gamedefine.getSaveData(from_dict(gamedefine.GameDefine, gamedefine.defualtGameDefine)))
        )
        blankSaveMetadata = json.dumps(
            gamedefine.getSaveMetadata(from_dict(gamedefine.GameDefine, gamedefine.defualtGameDefine))
        )
...
def load(slot: int = 0, noSpeak=False, loadWarn=True, save=None):
    dialog = dialogs.yesNoDialog(
        "Load", "Are you sure you want to load? This will overwrite your current save.", preventClose_=True
    )
