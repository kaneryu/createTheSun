...
def receive(event):
    ...
    for i in data.amounts:
        if not i in data.gainPerSecond:
            ...
            try:
                data.gainPerSecond[i] = round(
                    (data.amounts[i] - previous[i]) / ((time.time()) - data.lastCheckedTime), 3
                )
            except ZeroDivisionError:
                data.gainPerSecond[i] = round(
                    (data.amounts[i] - previous[i]) / ((time.time()) - data.lastCheckedTime - 1), 3
                )
...
itemObserver = observerModel.registerObserver(
    receive, observerModel.Observable.ITEM_OBSERVABLE, observerModel.ObservableCallType.ALL
)
